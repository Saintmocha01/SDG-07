...
export class SolarEnergyModel {
  ...
  predict(data: WeatherData): SolarPrediction {
    ...
    const optimalAngle = this.calculateOptimalAngle(data);
  ...
  private calculateOptimalAngle(data: WeatherData): number {
    const latitude = 35;
  ...
  batchPredict(dataPoints: WeatherData[]): SolarPrediction[] {
    return dataPoints.map(data => this.predict(data));
  }
}

export const solarModel = new SolarEnergyModel();
